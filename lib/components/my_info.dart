...
class _MyInfoState extends State<MyInfo> {
  ...
  Future<Adress?> readAdress() async {
    final docAdress =
        FirebaseFirestore.instance.collection('adresses').doc(user.id);
  ...
  Widget buildAdress(Adress endereco) {
    return Column(
      children: [
        const SizedBox(height: 40),
        buildInfo("CEP", endereco.zipCode ?? "Não cadastrado!"),
        const SizedBox(height: 40),
        buildInfo("Rua", endereco.street ?? "Não cadastrado!"),
        const SizedBox(height: 40),
        buildInfo("Bairro", endereco.district ?? "Não cadastrado!"),
        const SizedBox(height: 40),
        buildInfo("Número da casa", endereco.houseNumber ?? "Não cadastrado!"),
        const SizedBox(height: 40),
      ],
    );
  }
